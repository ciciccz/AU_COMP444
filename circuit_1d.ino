...
int khaki_rgb[] = { 195, 176, 145 };
int red_rgb[] = { 100, 0, 0 };
int bule_rgb[] = { 0, 0, 100 };
int green_rgb[] = { 0, 100, 0 };
...
void loop() {
  ...
  if (photoresistor < threshold) {  //if it's dark (the photoresistor value is below the threshold) turn the LED on
    ...
    if (potentiometer > 600 && potentiometer <= 750)
      cyan();
    if (potentiometer > 750 && potentiometer <= 800)  //will turn off between 800 - 900
      blue();
    if (potentiometer > 900)  //multiblink at past 900
      multiBlink();
